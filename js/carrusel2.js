...
window.addEventListener('DOMContentLoaded', () => {
  const contenedor = document.querySelector('.escenarios2');
  const slides = document.querySelectorAll('.escenarios-img');
  const prevBtn = document.querySelector('.prev2');
  const nextBtn = document.querySelector('.next2');
  const section = document.querySelector('.escenarios'); // para pausar al hover
